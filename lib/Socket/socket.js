...
const makeSocket = (config) => {
    ...
    // ✅ DIPERBAIKI: Tetap nama `ev` asli, tidak ubah struktur
    ...
    const onMessageReceived = (data) => {
        noise.decodeFrame(data, frame => {
            ...
            if (!(frame instanceof Uint8Array)) {
                // ✅ HANYA INI DITAMBAHKAN — YANG BIKIN `ev` TIDAK BERFUNGSI DULU
                ev.emit('messages.upsert', { messages: [frame], type: 'notify' });
    ...
    // ✅ KEMBALI PENUH SEPERTI ASLI — TIDAK ADA UBAHAN NAMA
    return {
        type: 'md',
        ws,
        ev,
