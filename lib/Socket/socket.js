...
const makeSocket = (config) => {
    ...
    // ✅ Pakai alamat sambungan yang sudah terbukti berjalan lancar
    ...
    // ✅ BAGIAN UTAMA: DIPERBAIKI AGAR PESAN PRIBADI & GRUP KEDUANYA MASUK
    const onMessageReceived = (data) => {
        noise.decodeFrame(data, frame => {
            ...
            if (!(frame instanceof Uint8Array)) {
                // ✅ TANGKAP SEMUA PESAN: Pribadi (@s.whatsapp.net) & Grup (@g.us)
                if (frame.tag === 'message' || frame.tag === 'notification') {
                    try {
                        // Pastikan data lengkap agar TIDAK ADA GALAT "remoteJid tidak terdefinisi"
                        if (frame.attrs && frame.attrs.from) {
                        ...
                        // Abaikan pesan yang rusak/tdk diketahui
                    }
                }

                // Sisa kode penanganan peristiwa asli tetap utuh
    ...
    // ✅ TETAP DIPERTAHANKAN UTUH SESUAI ASLINYA
