...
const makeSocket = (config) => {
    ...
    // ✅ URL RESMI BARU
    const url = new URL("wss://web.whatsapp.com/ws/chat");
    if ((_a = authState === null || authState === void 0 ? void 0 : authState.creds) === null || _a === void 0 ? void 0 : _a.routingInfo) {
    ...
    ws.on('CB:iq,type:set,pair-device', async (stanza) => {
        ...
        const genPairQR = () => {
            ...
            if (!refNode) { end(new boom_1.Boom('QR refs attempts ended', { statusCode: Types_1.DisconnectReason.timedOut })); return; }
    ...
    ws.on('CB:success', async (node) => {
        ...
        } catch (err) { logger.error({ err }, 'error opening connection'); end(err); }
    });
    ...
    ws.on('CB:ib,,offline_preview', (node) => { logger.info('offline preview received', JSON.stringify(node)); sendNode({ tag: 'ib', attrs: {}, content: [{ tag: 'offline_batch', attrs: { count: '100' } }] }); });
    ws.on('CB:ib,,edge_routing', (node) => {
        const edgeRoutingNode = (0, WABinary_1.getBinaryNodeChild)(node, 'edge_routing');
        const routingInfo = (0, WABinary_1.getBinaryNodeChild)(edgeRoutingNode, 'routing_info');
        if (routingInfo === null || routingInfo === void 0 ? void 0 : routingInfo.content) { authState.creds.routingInfo = Buffer.from(routingInfo === null || routingInfo === void 0 ? void 0 : routingInfo.content); ev.emit('creds.update', authState.creds); }
    });

    let didStartBuffer = false;
    process.nextTick(() => {
        var _a;
        if ((_a = creds.me) === null || _a === void 0 ? void 0 : _a.id) { ev.buffer(); didStartBuffer = true; }
    ...
    ev.on('creds.update', update => {
        ...
        if (((_b = creds.me) === null || _b === void 0 ? void 0 : _b.name) !== name) {
            ...
            sendNode({ tag: 'presence', attrs: { name } })
                .catch(err => logger.warn({ trace: err.stack }, 'error in sending presence update'));
        }
    ...
    return {
        ...
        generateMessageTag, query, waitForMessage, waitForSocketOpen, sendRawMessage, sendNode, logout, end, onUnexpectedError,
        uploadPreKeys, uploadPreKeysToServerIfRequired, requestPairingCode,
        waitForConnectionUpdate: (0, Utils_1.bindWaitForConnectionUpdate)(ev), sendWAMBuffer,
    };
};
