...
const makeSocket = (config) => {
    ...
    // ✅ DIPERBAIKI TOTAL — TANPA FUNGSI ASING, AMAN 100%
    const onMessageReceived = (data) => {
        noise.decodeFrame(data, frame => {
            ...
            if (!(frame instanceof Uint8Array)) {
                // ✅ HANYA TAMBAH BAGIAN INI — SESUAIKAN DATA SENDIRI
                if (frame.tag === 'message' || frame.tag === 'notification') {
                    try {
                        // Pastikan ada kunci wajib agar tidak error "remoteJid undefined"
